...
describe("Index page when awdeorio is logged in with mock database", () => {
  ...
  it("Updates human-readable timestamps every minute without page refresh", () => {
    // Freeze time at a fixed moment
    const now = new Date("2025-09-21T12:00:00Z");
    cy.clock(now.getTime());

    // Intercept feed with new post at the top
    cy.intercept("GET", "/api/v1/posts/", {
      fixture: "posts_page_0_recent.json",
    }).as("getPosts");

    // Intercept detailed endpoint for post 33
    cy.intercept("GET", "/api/v1/posts/33/", {
      fixture: "posts/33.json",
    }).as("getPost33");
    ...
    // Verify that the new post link exists
    cy.get("a[href='/posts/33/']").should("exist");
    ...
    // Timestamp should update to "1 minute ago"
    cy.get("[data-testid='post-time-ago']")
      .first()
      .invoke("text")
      .should((text) => {
        expect(text).to.match(/a minute ago/);
      });
  });
});
